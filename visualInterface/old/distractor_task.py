...
########################get input args########################
subject_number = input("Enter the subject ID: ")
subject_session = input("Enter the number of session: ")
subject_run = input("Enter the number of run: ")
# set_size = int(input("Enter set size: "))
...
HWTrigger.init(50)
bci = BCI_tid.BciInterface() ############## new
trial_index = 0
trigger=[]
responses = []
n_trials = 60
n_d_trials = n_trials/2
mid_d_trials = n_d_trials/3
lat_d_trials = n_d_trials - mid_d_trials
latd_latt_trials = int(lat_d_trials/10)
latd_midt_trials = int(lat_d_trials - latd_latt_trials)
...
#Generate the trial_type list; no more than 3 same-type trials in a row
...
vertical_d_pos = mid_pos * int(mid_d_trials/2) #2 * (10/2)
horizontal_d_pos = lat_pos * math.ceil((lat_d_trials)/len(lat_pos)) #8 * (20/8)
random.shuffle(horizontal_d_pos)
all_positions = vertical_d_pos + horizontal_d_pos[:20]
...
latd_latt_trials_pos = [random.choice(lat_pos) for i in range(latd_latt_trials)]
latd_midt_trials_pos = mid_pos * int(latd_midt_trials/2)
all_latd_t_pos = latd_latt_trials_pos + latd_midt_trials_pos
...
#             if d_pos[i] in lat_pos:
#                 available_positions = [pos for pos in all_latd_t_pos if pos != d_pos[i]]
#                 # Check if there are available positions
#                 if available_positions:
#                     t_pos[i] = random.choice(available_positions)
#                     all_latd_t_pos.remove(t_pos[i])
#                 else:
#                     raise ValueError("No available positions, reshuffling and restarting")
#             else:
#                 t_pos[i] = random.choice([pos for pos in range(1, set_size+1) if pos != d_pos[i]])
#         success = True

#     except ValueError as e:
#         random.shuffle(all_latd_t_pos)
lat_d_indices = [i for i, t in enumerate(trial_type) if t == 1 and d_pos[i] in lat_pos]
mid_d_indices = [i for i, t in enumerate(trial_type) if t == 1 and d_pos[i] in mid_pos]


# Check if there are enough target positions for lat_pos distractors
if len(lat_d_indices) > len(all_latd_t_pos):
    print("Not enough target positions for lat_pos distractors.")
    pygame.quit()
    sys.exit(1)

# Assign target positions for lat_pos distractors
for i in lat_d_indices:
    available_positions = [pos for pos in all_latd_t_pos if pos != d_pos[i]]
    if available_positions:
        chosen_pos = random.choice(available_positions)
        t_pos[i] = chosen_pos
        all_latd_t_pos.remove(chosen_pos)
...
# Assign target positions for mid_pos distractors
for i in mid_d_indices:
    available_positions = [pos for pos in range(1, set_size + 1) if pos != d_pos[i]]
    if available_positions:
        chosen_pos = random.choice(available_positions)
        t_pos[i] = chosen_pos
...
# Assign target positions for non-distractor trials
for i in range(n_trials):
    if trial_type[i] == 0:
        available_positions = [pos for pos in range(1, set_size + 1) if pos != d_pos[i]]
        if available_positions:
...
print("Trial assignments completed successfully.")
