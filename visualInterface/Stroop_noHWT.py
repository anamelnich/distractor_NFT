...
class StroopTask:
    ...
    def run_trial(self, stimulus, is_circle=False, ink_color=None, trial_type=None):
        ...
        if is_circle:
            ...
            # if tril_type == 'congruent':
            #     self.parallel.signal(10)
            # elif tril_type == 'incongruent':
            #     self.parallel.signal(20)
            # elif tril_type == 'neutral':
        ...
        if "text" in result and result["text"]:
            ...
            if "result" in result and len(result["result"]) > 0:
                ...
                reaction_time = first_word["start"]
            else:
                reaction_time = None
        ...
        trial_data = {
            ...
            "stimulus": stimulus,
            "ink_color": ink_color if ink_color else "N/A",
            "response": correct_response if correct_response is not None else (attempts[0][0] if attempts else "N/A"),
            "reaction_time": reaction_time if reaction_time is not None else "N/A",
            "correct": correct,
            "timeout": timeout,
    ...
    def save_data(self, part_number):
        ...
        with open(filename, 'w') as f:
            # Write header (added num_attempts column)
            f.write("Trial\tStimulus\tInk_Color\tResponse\tReaction_Time\tCorrect\tTimeout\tNum_Attempts\n")
