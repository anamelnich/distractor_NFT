...
% title('Raw signal')
%%
unique(training.trigger )
%%%%%%%%%%%%%%
%% Epoching %%
%%%%%%%%%%%%%%
epochs.data = nan(length(cfg.epochSamples), length(cfg.eegChannels), length(training.index.pos));
epochs.labels = training.index.typ; 
epochs.file_id = nan(length(training.index.typ), 1); 
epochs.file_type = cell(length(training.index.typ), 1);

for i_trial = 1:length(training.index.pos)
    epochs.data(:, :, i_trial) = training.data(training.index.pos(i_trial) + cfg.epochSamples, cfg.eegChannels);    
...
%% Calc averages %%
%%%%%%%%%%%%%%%%%
leftElectrodeIndices = [24,25,50,51,54,55,62];
rightElectrodeIndices = [27,28,52,53,56,57,63];

PO7_data_dright = mean(epochs.data(:, leftElectrodeIndices, epochs.labels == 1), 3);
PO8_data_dright = mean(epochs.data(:, rightElectrodeIndices, epochs.labels == 1), 3);
PO8_data_dleft = mean(epochs.data(:, rightElectrodeIndices, epochs.labels == 2), 3);
PO7_data_dleft = mean(epochs.data(:, leftElectrodeIndices, epochs.labels == 2), 3);
PO7_data_dnone = mean(epochs.data(:, leftElectrodeIndices, epochs.labels == 0), 3);
PO8_data_dnone = mean(epochs.data(:, rightElectrodeIndices, epochs.labels == 0), 3);

electAvg_dright = mean(PO7_data_dright,2);
electAvg_dleft = mean(PO8_data_dleft,2);
electAvg_dright_ipsi = mean(PO8_data_dright,2);
electAvg_dleft_ipsi = mean(PO7_data_dleft,2);
electAvg_dnone_left = mean(PO7_data_dnone,2);
electAvg_dnone_right = mean(PO8_data_dnone,2);

dif_dright = electAvg_dright - electAvg_dright_ipsi;
dif_dleft = electAvg_dleft - electAvg_dleft_ipsi;
dif_dnone_rl = electAvg_dnone_right - electAvg_dnone_left;
dif_dnone_lr = electAvg_dnone_left - electAvg_dnone_right;
...
plot(cfg.epochTime, electAvg_dright, 'Color', cfg.plotColor{1}, cfg.plotOption{:}); hold on;
plot(cfg.epochTime, electAvg_dleft, 'Color', cfg.plotColor{2}, cfg.plotOption{:}); hold on;
plot(cfg.epochTime, electAvg_dnone_left, 'Color', cfg.plotColor{5}, cfg.plotOption{:}); hold on;
%plot(cfg.epochTime, electAvg_dnone_right, 'Color', cfg.plotColor{5}, cfg.plotOption{:},'LineStyle','--');
xlabel('Time (s)')
ylabel('Amplitude (\muV)');
xline(0);
yline(0);
xlim([-0.1 0.6]); 
ylim([-15 15])
legend('right contra','left contra','none left' );
...
figure('Name','ERP Difference wave')
plot(cfg.epochTime, dif_dright, 'Color', cfg.plotColor{1}, cfg.plotOption{:}); hold on;
plot(cfg.epochTime, dif_dleft, 'Color', cfg.plotColor{2}, cfg.plotOption{:}); hold on;
plot(cfg.epochTime, dif_dnone_rl, 'Color', cfg.plotColor{5}, cfg.plotOption{:}); hold on;
plot(cfg.epochTime, dif_dnone_lr, 'Color', cfg.plotColor{5}, cfg.plotOption{:},'LineStyle','-'); 
xlabel('Time (s)')
ylabel('Amplitude (\muV)');
xline(0);
yline(0);
xlim([-0.1 0.6]); 
ylim([-15 15])
legend('dright', 'dleft','dnone r-l','dnone l-r');
...
epochs.newlabels = epochs.labels;
epochs.newlabels(epochs.labels == 2) = 1;
...
epochs.posteriors = singleClassification(decoder, epochs.data, epochs.labels, 0, decoder.leftElectrodeIndices,decoder.rightElectrodeIndices);
%%
