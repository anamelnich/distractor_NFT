...
function [calibration, validation, training, decoding] = loadData(path)
...
calibration = []; validation = []; training = []; decoding = []; 
...
for i = 1:length(dirNames)
    ...
    elseif (validationFile(i))
        if (isfield(validation, 'data'))
            validation.header.EVENT.TYP = cat(1, validation.header.EVENT.TYP, header.EVENT.TYP);
            validation.header.EVENT.POS = cat(1, validation.header.EVENT.POS, header.EVENT.POS + size(validation.data, 1));
            validation.data = cat(1, validation.data, signal);
            validation.eof = cat(1, validation.eof, size(validation.data, 1));
        else
            validation.data = signal;
            validation.header = header;
            validation.eof = size(signal, 1);
        end
