function boxplotStyle(data, groups, varargin)
if nargin == 2
    b = boxplot(data, groups, 'symbol','');
elseif nargin > 2
    b = boxplot(data, groups, 'symbol','', 'Positions', varargin{1});
end
...
for i = 1:length(uniqueGroups)
    mn(i) = mean(data(groups == uniqueGroups(i)),1);
end
...
for i = 1:length(mn)
    ...
    if nargin == 2
        scatter(i+0.2*(rand(sum(groups == uniqueGroups(i)),1)-0.5),data(groups == uniqueGroups(i)),70,'filled','MarkerFaceColor',dots_color);
    elseif nargin > 2
