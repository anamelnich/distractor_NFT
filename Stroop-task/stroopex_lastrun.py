...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/hililbby/Downloads/Stroop-task-master/stroopex_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions = data.Routine(
        name='instructions',
        components=[instrText, key, key_resp_2],
    )
    ...
    while continueRoutine:
        ...
        if key.status == STARTED and not waitOnFlip:
            theseKeys = key.getKeys(keyList=None, ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 2.2:
            ...
            if response.status == STARTED and not waitOnFlip:
                theseKeys = response.getKeys(keyList=['left','right','down'], ignoreKeys=["escape"], waitRelease=False)
